...
**Pi.dev**: no native MCP support (deliberate design choice). A community adapter exists at [pi-mcp-adapter](https://github.com/nicobailon/pi-mcp-adapter) but is unmaintained. Wevex does not auto-detect Pi.
...
`%APPDATA%\wevex\` (the same set of files macOS/Linux keep in
`~/.config/wevex/`). To see or remove the task by hand:
...
nohup-style spawn — survives terminal close but not reboot. Run `wevex up`
...
`wevex up` runs the initial ingest and registers a file-watcher that
...
in `wevex doctor` (alongside fragment counts, value distribution, and
inbox depth). One-off cleanup is `wevex doctor --clean`.
...
Scopes are inferred automatically — `wevex up` picks one from
`.wevex/scope`, the git remote, or the directory name. Sub-scopes
