...
_GLOB_PATTERNS: tuple[str, ...] = (
    ...
    "architecture/**/*.md",
    ".skein/docs/**/*.md",
    # Iter 29 day-one: AI-tool config files often carry the project's
    # accumulated rules, decisions, and preferences. A brand-new Skein
    # install on an existing project should harvest them so `recall`
    # returns real signal from minute zero instead of an empty store.
    # AGENTS.md is the universal one; CLAUDE.md / .cursor/rules / .clinerules
    # cover the three biggest single-tool installs.
    "AGENTS.md",
    "CLAUDE.md",
    ".cursor/rules/**/*.mdc",
    ".cursor/rules/**/*.md",
...
def _process_file(repo: Path, path: Path) -> list[ScannedFact]:
    ...
    # Iter 29 day-one: skip Skein-generated AGENTS.md files so the docs
    # watcher doesn't ingest its own output and create an inflation loop
    # (AGENTS.md fragments → docs sweep → more fragments → bigger AGENTS.md →
    # …). The auto-sync loop stamps the file with a known header; if we
    # see it, the file's content is already in the fragment store via the
    # original sources.
    if "Generated by Skein" in text[:200]:
        return []
